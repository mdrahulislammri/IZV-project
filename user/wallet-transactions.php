...
$stmt = $pdo->prepare('SELECT type, amount, note, created_at FROM user_wallet_transactions WHERE user_id=? ORDER BY id DESC');
$stmt->execute([$user['id']]);
$rows = $stmt->fetchAll();
?>
<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>User Wallet Transactions</title><script src="https://cdn.tailwindcss.com"></script></head>
<body class="bg-slate-950 text-slate-100"><div class="max-w-5xl mx-auto p-4 sm:p-6"><div class="mb-4 flex items-center justify-between"><h1 class="text-3xl font-black">Wallet Transactions</h1><a class="text-blue-400" href="/user/wallet">Back to Wallet</a></div><p class="mb-3 text-slate-300">Current Balance: <b>৳<?= number_format($balance,2) ?></b></p>
