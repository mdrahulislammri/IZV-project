...
?>
<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Invoices</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-slate-100"><div class="max-w-7xl mx-auto p-4 sm:p-6">
<h1 class="text-3xl font-bold mb-4">Invoice</h1>
<div class="bg-white border rounded-xl overflow-x-auto"><table class="w-full min-w-[760px] text-sm"><thead><tr class="bg-slate-50"><th class="p-3 text-left">Website</th><th>Domain</th><th>Duration</th><th>Expiry</th><th>Price</th><th>Status</th><th>Action</th></tr></thead><tbody>
<?php foreach($invoices as $i): ?>
<tr class="border-t"><td class="p-3"><?= htmlspecialchars($i['website_name']) ?></td><td><?= htmlspecialchars($i['domain_name']) ?></td><td><?= (int)$i['duration_months'] ?> months</td><td><?= htmlspecialchars($i['due_date']) ?></td><td>৳<?= number_format((float)$i['total'],2) ?></td><td><?= htmlspecialchars($i['order_status']) ?></td><td><a class="text-blue-600" href="/user/invoice?renew=<?= (int)$i['id'] ?>">Renew</a></td></tr>
<?php endforeach; ?>
</tbody></table></div>
</div></body></html>
